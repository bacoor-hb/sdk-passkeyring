...
const oldGroupSlug = 'egglepasskeywallet-decard'
...
fs.readFile(constantsFilePath, 'utf8', (err, data) => {
  ...
  const updatedData = data.replace(`export const GROUP_SLUG: SlugGroup = '${oldGroupSlug}'`, `export const GROUP_SLUG: SlugGroup = '${newGroupSlug}'`)
  ...
  fs.writeFile(constantsFilePath, updatedData, 'utf8', (err) => {
    ...
    // Bước 2: Tìm kiếm và thay đổi lại hết tất cả trong source cụm từ "sdk-v2-egglepasskeywallet-decard" thành "egglepasskeywallet"
    const directoryPath = path.join(__dirname, 'lib')
    const filesToUpdate = [
      path.join(__dirname, 'README.md'),
      path.join(__dirname, 'package.json'),
    ]
    const searchString = 'sdk-v2-egglepasskeywallet-decard'
    const replaceString = 'sdk-v2-egglepasskeywallet'
